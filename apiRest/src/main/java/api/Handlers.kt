...
data class UserProfile(val firstName: String, val lastName: String, val password: String, val email: String, val balance: Double, val cvu: String) {
    constructor(user: User) : this(user.firstName, user.lastName, user.password, user.email, user.account?.balance!!, user.account?.cvu!!)
}
