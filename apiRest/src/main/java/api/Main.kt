...
fun main(args: Array<String>) {
    ...
    app.get("/") { ctx -> ctx.result("viva peron") }
    ...
    app.post("user/") {
        val dataToEditBody = it.bodyAsClass(UserProfile::class.java);
        try {
            for (usuario in digitalWallet.users) {
                if (usuario.account?.cvu!! == dataToEditBody.cvu) {
                    usuario.firstName = dataToEditBody.firstName;
                    usuario.lastName = dataToEditBody.lastName;
                    usuario.email = dataToEditBody.email;
                    usuario.password = dataToEditBody.password;
                    it.json("Usuario Modificado Con Éxito.")
                }
            }
        } catch (exception: Exception) {
            it.status(400)
